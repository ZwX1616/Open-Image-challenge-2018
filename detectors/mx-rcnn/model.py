...
def load_param(params, ctx=None):
    ...
    for k, v in save_dict.items():
        # if not 'conv' in k and not 'conv' in k:
        tp, name = k.split(':', 1)
        if tp == 'arg':
...
def initialize_frcnn(symbol, data_shapes, arg_params, aux_params):
    ...
    for key in arg_shape_dict.keys():
        if 'conv0' in key or 'conv1' in key or 'conv2' in key or 'conv3' in key or '_sc' in key:
            if 'weight' in key:
                arg_params[key] = mx.random.normal(0, 0.01, shape=arg_shape_dict[key])
            elif 'bias' in key:
                arg_params[key] = mx.nd.zeros(shape=arg_shape_dict[key])
            else:
                print(str(key)+' is not either weight or bias')
    ...
    if fixed_param_prefix:
        for name in symbol.list_arguments():
            for prefix in fixed_param_prefix:
                if prefix in name and not 'conv' in name:
