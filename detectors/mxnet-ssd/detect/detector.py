...
class Detector(object):
	...
	def detect_and_save_result (self, im_list, root_dir, subset, extension=None,
							 classes=[], thresh=0.6, show_timer=False):
		...
		if not os.path.isdir('/root/predictions/series_'+subset):
			os.mkdir('/root/predictions/series_'+subset) 
		...
		for k, det in enumerate(dets):
			...
			for i in range(det.shape[0]):
				if det[i][1]>=thresh:
					lbl_str+=classes[int(det[i][0])]+' '+'{:.2f}'.format(det[i][1])+' '+'{:.2f}'.format(det[i][2])+' '+'{:.2f}'.format(det[i][3])+' '+'{:.2f}'.format(det[i][4])+' '+'{:.2f}'.format(det[i][5])+' '
			
			with open('/root/predictions/series_'+subset+'/'+im_list[k][-20:-4]+'.txt','w+', newline='') as wf:
				if len(lbl_str)>0:
					lbl_str=lbl_str[:-1]
					wf.write(lbl_str)