...
#    W=200
#    H=100
    img=pilimg.open('../sources/train_img/'+img_id+'.jpg')
    W=img.size[0]
    H=img.size[1]
    ...
    if H>W:
        ...
        isRotated=False
    # print(isRotated)
    size_dict={'width':str(512),'height':str(384)}
#    size_dict={'width':str(W),'height':str(H)}
    ...
    for bbox in bboxs:
        if isRotated==False:
            xy_dict={'xmin':str(float(bbox[1])*512),'ymin':str(float(bbox[2])*384),'xmax':str(float(bbox[3])*512),'ymax':str(float(bbox[4])*384)}
        else:
            xy_dict={'xmin':str(float(bbox[2])*512),'ymin':str(384-float(bbox[3])*384),'xmax':str(float(bbox[4])*512),'ymax':str(384-float(bbox[1])*384)}
    ...
    with open('../devkit_person/labels/'+img_id+'.xml', 'w+') as f:
        f.write(out)

# ---main--- #
start=time()
with open('../devkit_person/labels/train-bbox-people.csv') as cfile:
...
for i,rows in enumerate(readeritem):
		...
		if i>0:
			...
			diff=1-int(rows[3])
			bboxs.append(['person',rows[4],rows[6],rows[5],rows[7],str(diff)])
